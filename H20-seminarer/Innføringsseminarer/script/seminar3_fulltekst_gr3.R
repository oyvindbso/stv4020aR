...
ggplot(snitt_data, aes(x = institutional_quality, y = fit.fit)) + # Setter institusjonell kvalitet på x-aksen og predikert verdi på y-aksen
  ...
  geom_ribbon(aes(ymin = fit.lwr, ymax = fit.upr, color = NULL), alpha = 0.2) + # Legger til konfidensintervall på plottet
  labs(x = "Kvalitet på institusjoner", y = "Forventet GDP vekst") # Setter tittel på akser og plot
