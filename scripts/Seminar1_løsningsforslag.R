...
##### med løsningsforslag på oppgaver          #####
####################################################


## Merk: løsningsforslag til de separate oppgavene
## finnes på slutten av dokumentet til seminar 2
...
#løsningsforslag:
summary(aid$policy - mean(aid$policy, na.rm = T)) # skal opprettes som objekt
summary(aid$policy2 - mean(aid$policy2, na.rm = T))
...
# Kan gjerne lagre endringene i objektet, jeg gjør det ikke her
# Viktig å opprette tre nye variabler!
aid %>% 
  mutate(
    period_fac = as.factor(period),
    country_fac = as.factor(country),
    aid_char = as.character(elraid)
  )
...
aid$mixed_pol <- ifelse(aid$policy>0 & aid$policy2<0, 1, 0)
table(aid$mixed_pol) # en god del land har blanding.
...
new_data <- aid %>%
  group_by(region) %>%
  summarise(
    aid_gdpg = cor(elraid, elrgdpg, use= "complete.obs"), # Fjerner missing.
    aid_pol = cor(elraid, policy, use= "complete.obs"),
    gdpg_pol = cor(elrgdpg, policy, use= "complete.obs")
  )

new_data # Er noen forskjeller, se for eksempel kolonnen med aid_pol

# del 2:

aid$good_pol <- ifelse(aid$policy>0, 1, 0)
aid$good_pol2 <- ifelse(aid$policy2 >0 , 1, 0)

new_data2 <- aid %>% 
  group_by(good_pol, good_pol2) %>%
  summarise(aid_gdpg = cor(elraid, elrgdpg, use= "complete.obs"))
